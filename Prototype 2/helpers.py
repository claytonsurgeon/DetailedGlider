...
def normalize(data, wave_data, output, sampling_freq, samples):
    ...
    spectrum = []
    if(output == 'power'):
        spectrum = (data.conjugate() * data).real / \
            (wave_data.size * sampling_freq)
        spectrum[1:-1] *= 2
...
def windowfft(data, M, sample_freq, type):
    ...
    denominator = hann_window.sum() * sample_freq   # NEVER USED
    for window in windows:
        A = np.fft.rfft(window*hann_window)
        spectrum = calcPSD(A, A, sample_freq).real
        spectrums += spectrum

    # should denominator go here?
